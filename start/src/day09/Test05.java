...
public class Test05 {
	...
	public void toPrint(int no1, int no2, int result) {
		System.out.println("두 수 "+no1+"와"+ no2+"의 합은"+result+"입니다");
	}
	public void toPrint(int no1, int no2) {
		System.out.println("두 수 "+no1+"와"+ no2+"의 합은"+add(no1,no2)+"입니다");
	}
	/*
		이 경우 "의 합은" 까지 출력하고 함수를 실행하는 것이 아니고 먼저add()함수를 호출한 후에 문자열을 만들어서 그 후에 출력하게 된다.
	*/
}
